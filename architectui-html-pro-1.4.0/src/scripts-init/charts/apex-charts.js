...
// ── Stacked Bar — Cantidad de Alumnos por Curso (Analytics Dashboard) ──────

var optionsAlumnosCurso = {
    chart: {
        type: 'bar',
        height: 340,
        stacked: true,
    ...
    colors: ['#423ffe', '#fe4a6b', '#86909c'],
    series: [
        {
            name: 'Masculino',
            data: [18, 22, 15, 30, 12, 25, 19, 28, 14, 21, 17, 26, 11, 23, 16, 29, 13, 20, 24, 18, 27, 15]
        },
        {
            name: 'Femenino',
            data: [20, 18, 22, 14, 28, 16, 24, 12, 30, 19, 21, 11, 27, 15, 23, 13, 25, 17, 10, 22, 16, 20]
        },
        {
            name: 'Otro',
            data: [3, 2, 4, 1, 3, 2, 1, 3, 2, 4, 2, 3, 1, 2, 3, 2, 4, 3, 2, 3, 2, 4]
        }
    ],
    xaxis: {
        categories: [
            'Matemática I', 'Matemática II', 'Física I', 'Física II', 'Química I', 'Química II',
            'Historia', 'Geografía', 'Literatura', 'Inglés I', 'Inglés II', 'Artes Plásticas',
            'Música', 'Ed. Física', 'Informática I', 'Informática II', 'Biología', 'Ecología',
            'Filosofía', 'Psicología', 'Sociología', 'Economía'
        ],
        tickAmount: 22,
        labels: {
            rotate: -45,
            ...
            formatter: function(val) {
                ...
                // Si el nombre termina en sufijo romano (" I", " II", " III"…)
                // preservar el sufijo para que el label sea único
                var match = val.match(/^(.+?)(\s+[IVX]+)$/);
                if (match) {
                    var base = match[1];
                    var suffix = match[2]; // ej: " I" o " II"
                    var available = 6 - suffix.length;
                    return available > 0 ? base.substring(0, available) + suffix : suffix.trim();
                }
                // Sin sufijo: truncar y agregar ellipsis
                return val.substring(0, 6).trimRight() + '…';
            },
            style: {
                fontSize: '11px',
                fontFamily: 'Poppins, sans-serif',
                colors: Array(22).fill('#86909c'),
            }
        },
    ...
    tooltip: {
        ...
        x: {
            // Muestra el nombre completo de la categoría (sin truncar)
            formatter: function(val, opts) {
                var cats = opts.w.config.xaxis.categories;
                return cats && cats[val - 1] ? cats[val - 1] : val;
            }
        }
    },
    yaxis: {
    ...
    legend: {
        show: false, // reemplazada por leyenda HTML debajo del gráfico
    },
    grid: {
        borderColor: '#eaeef2',
        strokeDashArray: 4,
...
// ── Pie — Distribución por género (Analytics Dashboard) ─────────────────────
var optionsGeneroChart = {
    chart: {
        type: 'pie',
        height: 340,
        fontFamily: 'Poppins, sans-serif',
...
$(document).ready(() => {
    ...
    setTimeout(function () {
        ...
        if (document.getElementById('chart-apex-alumnos-curso')) {
            new ApexCharts(document.querySelector('#chart-apex-alumnos-curso'), optionsAlumnosCurso).render();
        }

        if (document.getElementById('chart-apex-genero')) {
            new ApexCharts(document.querySelector('#chart-apex-genero'), optionsGeneroChart).render();
        }
